...
func setupGuildChatTest(t *testing.T) (*connection.Server, *connection.Client, *connection.Client) {
	...
	os.Setenv("MODE", "test")
	orm.InitDatabase()
	execAnswerTestSQLT(t, "TRUNCATE TABLE guild_chat_messages RESTART IDENTITY CASCADE")
	execAnswerTestSQLT(t, "TRUNCATE TABLE commanders RESTART IDENTITY CASCADE")
...
func TestGuildSendMessageBroadcasts(t *testing.T) {
	...
	if _, _, err := answer.GuildSendMessage(&buffer, client); err != nil {
...
func TestCommanderGuildChatHistory(t *testing.T) {
	...
	if _, err := orm.CreateGuildChatMessage(0, client.Commander.CommanderID, "first", base); err != nil {
		t.Fatalf("create message 1: %v", err)
	}
	if _, err := orm.CreateGuildChatMessage(0, client.Commander.CommanderID, "second", base.Add(2*time.Minute)); err != nil {
	...
	if _, _, err := answer.CommanderGuildChat(&buffer, client); err != nil {
